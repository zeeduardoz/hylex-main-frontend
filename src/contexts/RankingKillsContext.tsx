...
export const RankingKillsProvider: React.FC = ({ children }: any) => {
  ...
  async function GetRanking(data: string) {
    ...
    const response = await api.get(
      `/utils/ranking/kills?perPage=${10}&currentPage=${page}&gameType=${data}`
    )
  ...
  async function GetRankingPrev() {
    ...
    const response = await api.get(
      `/utils/ranking/kills?perPage=${10}&currentPage=${
        page - 1
      }&gameType=${gameType}`
    )
